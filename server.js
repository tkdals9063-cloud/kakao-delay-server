// server.js
const express = require("express");
const axios = require("axios");
const app = express();

app.use(express.json());

// 🔑 카카오 봇 토큰 (KakaoAK ...)
const KAKAO_BOT_TOKEN = "YOUR_KAKAO_BOT_RESTAPI_KEY";

// 30분(1800초)
const DELAY = 1800 * 1000;

// ─────────────────────────────────────────
// Sleep 함수 (딜레이용)
// ─────────────────────────────────────────
function sleep(ms) {
  return new Promise(resolve => setTimeout(resolve, ms));
}

// ─────────────────────────────────────────
// POST /delay  (카카오 스킬 서버 엔드포인트)
// ─────────────────────────────────────────
app.post("/delay", async (req, res) => {
  console.log("🟡 딜레이 스킬 요청 들어옴");

  const userKey = req.body.userRequest.user.id;
  const workType = req.body.action?.params?.workType;

  console.log("👉 userKey:", userKey);
  console.log("👉 workType:", workType);

  // 👉 카카오가 **5초 안에 응답을 받아야 하기 때문에**
  // 즉시 성공 응답을 먼저 보냄
  res.json({
    version: "2.0",
    resultCode: "OK",
    output: {}
  });

  // ⚠ 30분 대기
  await sleep(DELAY);

  // ─────────────────────────────────────────
  // 근무 형태에 따라 전송 메시지 분기
  // ─────────────────────────────────────────
  let message = "";

  if (workType === "sit") {
    message =
      "💺 오래 앉아계셨네요!\n\n" +
      "30초면 가능한 간단 스트레칭 추천드릴게요 😊\n" +
      "1) 고관절 신전 스트레칭\n" +
      "2) 흉추 열기 스트레칭\n" +
      "3) 목 앞쪽 늘려주기\n\n" +
      "지금 바로 한 번 해볼까요?";
  } else if (workType === "stand") {
    message =
      "🚶 오래 서 있으면 하체가 많이 뭉쳐요!\n\n" +
      "20초 스트레칭 추천드립니다 👍\n" +
      "1) 고관절 굴곡 스트레칭\n" +
      "2) 스탠딩 햄스트링 스트레칭\n" +
      "3) 발목 회전 운동";
  } else {
    message = "30분 동안 근무 중이시네요! 스트레칭 한번 해보는 건 어떨까요?";
  }

  // ─────────────────────────────────────────
  // 30분 뒤 카카오 메시지 전송
  // ─────────────────────────────────────────
  try {
    await axios({
      method: "post",
      url: "https://kapi.kakao.com/v2/bot/message/send",
      headers: {
        "Content-Type": "application/json",
        Authorization: `KakaoAK ${KAKAO_BOT_TOKEN}`
      },
      data: {
        user_key: userKey,
        template_object: {
          object_type: "text",
          text: message,
          link: { web_url: "https://kakao.com" },
          buttons: [
            {
              label: "근무 재시작",
              action: "block",
              blockId: "YOUR_RESTART_BLOCK_ID",
              extra: { workType }
            },
            {
              label: "근무 종료",
              action: "message",
              messageText: "오늘도 고생 많으셨어요! 😊"
            }
          ]
        }
      }
    });

    console.log("🟢 30분 뒤 메시지 전송 완료");
  } catch (err) {
    console.error("🔴 카카오 메시지 전송 실패:", err.response?.data || err);
  }
});

// ─────────────────────────────────────────
// 기본 GET (서버 살아있는지 체크용)
// ─────────────────────────────────────────
app.get("/", (req, res) => {
  res.send("Server is alive");
});

// ─────────────────────────────────────────
// 서버 실행
// ─────────────────────────────────────────
const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log("🚀 Server running on port", PORT);
});
